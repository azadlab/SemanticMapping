#ifndef SEMVIZGLUT_HH
#define SEMVIZGLUT_HH
...
class SemVizGlutColorPoint {
public:
  SemVizGlutColorPoint() { }
  SemVizGlutColorPoint(float x, float y, float z, float R, float G, float B) {
...
class SemVizGlutLine {
public:
  SemVizGlutLine() { }
  SemVizGlutLine(float x1, float y1, float z1, float x2, float y2, float z2) {
...
class SemVizGlutSphere {
public:
  SemVizGlutSphere() { }
  SemVizGlutSphere(float x, float y, float z,float R, float G, float B) {
    pos[0] = x; pos[1] = y; pos[2] = z;
    col[0] = R; col[1] = G; col[2] = B;
...
class SemVizGlutColor4f {
public:
  GLfloat R,G,B,A;
};

//! Interface class for all drawable objects
class SemVizGlutObject
{
public:
  //! Draw
  virtual void draw() = 0;
...
class SemVizGlutPointCloudColor : public SemVizGlutObject
{
public:
  SemVizGlutPointCloudColor() {
  ...
  void push_back(float x, float y, float z, float R, float G, float B) {
      if(x!=x||y!=y||z!=z||R!=R||G!=G||G!=G||isinf(x)||isinf(y)||isinf(z))
          std::cout<<"INVALID POINT DATA"<<std::endl;
    pts.push_back(SemVizGlutColorPoint(x,y,z,R,G,B));
  }
  SemVizGlutColorPoint& getPoint(size_t i) { return pts[i]; }
private:
  float m_pointSize;
  std::vector<SemVizGlutColorPoint> pts;
};

class SemVizGlutSpheres: public SemVizGlutObject
{
public:
  SemVizGlutSpheres() {

  }
  void inline setRadius(float rad){radius=rad;}
  void draw() {

    for (size_t i = 0; i < spheres.size(); i++) {
      glColor3fv(spheres[i].col);
      glVertex3fv(spheres[i].pos);
      ...
      glPushMatrix();
      glTranslatef(spheres[i].pos[0],spheres[i].pos[1],spheres[i].pos[2]);
      glPopMatrix();
  ...
  void push_back(float x, float y, float z, float R, float G, float B) {
    spheres.push_back(SemVizGlutSphere(x,y,z,R,G,B));
  }
  SemVizGlutSphere& getSphere(size_t i) { return spheres[i]; }
private:
  float radius;
  std::vector<SemVizGlutSphere> spheres;
};

class SemVizGlutSetOfLines : public SemVizGlutObject
{
public:
  SemVizGlutSetOfLines() {
    setColor4(1.0, 0.4, 0.0, 1.0);
    m_antiAliasing = true;
  }
  void draw() {
...
private:
  float m_lineWidth;
  SemVizGlutColor4f m_color;
  bool m_antiAliasing;
  float m_pointSize;
  std::vector<SemVizGlutLine> lines;
};



class SemVizGlutEllipsoid : public SemVizGlutObject
{
public:
  SemVizGlutEllipsoid() {
    m_quantiles = 3;
  ...
  SemVizGlutEllipsoid(float quantiles, float lineWidth, bool drawSolid, int segments) {
...
class SemVizGlutEllipsoids : public SemVizGlutObject {
public:
  void draw() {
  ...
  void push_back(const SemVizGlutEllipsoid &obj) { objs.push_back(obj); }
  void clear() { objs.clear(); objs.resize(0); }
private:
  std::vector<SemVizGlutEllipsoid, Eigen::aligned_allocator<SemVizGlutEllipsoid> > objs;
};

//! Interface class
class SemVizGlutCamera {
public:
...
class SemVizGlutXYOrbitCamera : public SemVizGlutCamera {
private: 
  int last_button_;
  int last_state_;
...
public:
  ...
  void inline setOrigin(float x,float y,float z) {origin_x = x;origin_y=y;origin_z=z;}
  ...
  void addObject(SemVizGlutObject* object) {
...
protected:
  ...
  // // Objects to draw
  std::vector<SemVizGlutObject*> objects;
  SemVizGlutXYOrbitCamera camera;
...
class SemVizGlutCuboid : public SemVizGlutObject
{
public:
  SemVizGlutCuboid() {
    m_quantiles = 2*sqrt(9);
    m_lineWidth = 1;;
    m_drawSolid3D = true;
    m_3D_segments = 20;
    this->setColor4(0.5, 1.0, 0.1, 0.8);
  }
  SemVizGlutCuboid(float quantiles, float lineWidth, bool drawSolid, int segments) {
    m_quantiles = quantiles;
    m_lineWidth = lineWidth;
    m_drawSolid3D = drawSolid;
    m_3D_segments = segments;
    ...
    if(m_eigVal(0,0) != 0.0 && m_eigVal(1,1) != 0.0 && (dim==2 || m_eigVal(2,2) != 0.0) && m_quantiles!=0.0)
      {
        ...
        if (dim==2)
        ...
        else
          {
            ...
            mat[0] = m_eigVec(0,0); mat[1] = m_eigVec(1,0); mat[2] = m_eigVec(2,0);
            mat[4] = m_eigVec(0,1); mat[5] = m_eigVec(1,1); mat[6] = m_eigVec(2,1);
            mat[8] = m_eigVec(0,2); mat[9] = m_eigVec(1,2); mat[10] = m_eigVec(2,2);
...
//std::cout<<"DRAWING OBJECT WITH eigenval(0):"<<m_eigVal(0,0)<<", Quantile="<<m_quantiles<<std::endl;
//            gluSphere( obj, 1,m_3D_segments,m_3D_segments);
            glutSolidCube(1.0);
            checkOpenGLError();
...
class SemVizGlutCuboids : public SemVizGlutObject {
public:
  void draw() {
    for (size_t i = 0; i < objs.size(); i++) {
      objs[i].draw();
  ...
  void push_back(const SemVizGlutCuboid &obj) { objs.push_back(obj); }
  void clear() { objs.clear(); objs.resize(0); }
private:
  std::vector<SemVizGlutCuboid, Eigen::aligned_allocator<SemVizGlutCuboid> > objs;
};
