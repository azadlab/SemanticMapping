...
//		cvReleaseImage(&tobeshow);
		// Step Two. Cluster
		// Connect
std::cout<<"Step2: img hieght="<<img->height<<" Image width:"<<img->width<<std::endl;
		int regionCount = 0;
...
std::cout<<"Alive here"<<std::endl;
        ...
        for(int i=0;i<img->width*img->height*sizeof(int);i++)  modePointCounts=0;

std::cout<<"Before Mem allocation"<<std::endl;
		float *mode = new float[img->height*img->width*3];
        /*std::cout<<"After Mem allocation"<<std::endl;
		{
			int label = -1;
			for(int i=0;i<img->height;i++) 
				for(int j=0;j<img->width;j++)
